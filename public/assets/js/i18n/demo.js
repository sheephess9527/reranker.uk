window.I18N_PAGE = { zh: {
  ...
  "Try a scenario <span class=\"hint\">— a realistic query mixed with distractor passages</span>": "试一个示例场景 <span class=\"hint\">— 真实查询 + 混入的干扰段落</span>",
  "Query": "查询（Query）",
  ...
  "<strong>Tips:</strong> the built-in scenarios each mix one or two clearly off-topic lines in with the relevant ones — watch them sink to the bottom (red) while the on-topic passages rise (green). The harder <em>Legal clause</em> example packs in several passages that are all about termination, so only a precise cross-encoder picks the right one. Try rephrasing the query to see scores shift. First load is slower because of the model download; subsequent runs reuse the cached weights.": "<strong>小提示：</strong>每个内置场景都混入了一两段明显跑题的文本 —— 看它们沉到底部（红色），而切题的段落上浮（绿色）。更难的<em>法律条款</em>示例里塞了好几段都在讲「终止」的文本，只有精确的 cross-encoder 才能挑出正确的那一段。试着改写查询，观察分数如何变化。首次加载较慢是因为要下载模型；之后的运行会复用缓存的权重。",
